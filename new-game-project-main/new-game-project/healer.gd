...
@export var heal_radius: float = 5.0  # New: Radius within which enemies will be healed
@export var heal_amount: float = 20.0  # New: Amount of healing provided
...
func _ready() -> void:
	...
	# Set health type for healer enemy
	if health_component:
		health_component.enemy_type = health_component.EnemyType.BASIC  # You might want to add a HEALER type
